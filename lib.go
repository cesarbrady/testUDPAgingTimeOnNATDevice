...
func kcpListen(host string, port int, key string, salt string) kcpServerSideListener {
	block, err := kcp.NewAESBlockCrypt(pbkdf2.Key([]byte(key), []byte(salt), 1024, 32, sha1.New))
	if err != nil {
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> " + err.Error() + " >> " + fmtDebugStack(string(debug.Stack())))
	}
...
func (m *kcpServerSideConn) send(str string) {
	_, err := m.conn.Write([]byte(str))
	if err != nil {
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> " + err.Error() + " >> " + fmtDebugStack(string(debug.Stack())))
...
func (m *kcpServerSideConn) recv(buffersize int) string {
	reply := make([]byte, buffersize)
	n, err := m.conn.Read(reply)
	if err != nil {
		_, fn, line, _ := runtime.Caller(0)
...
func kcpConnect(host string, port int, key string, salt string) *kcpClientSideConn {
	...
	return &kcpClientSideConn{conn: conn}
}

func (m *kcpClientSideConn) send(str string) {
	_, err := m.conn.Write([]byte(str))
	if err != nil {
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> " + err.Error() + " >> " + fmtDebugStack(string(debug.Stack())))
...
func (m *kcpClientSideConn) recv(buffersize int) string {
	reply := make([]byte, buffersize)
	//m.conn.SetReadDeadline()
	n, err := m.conn.Read(reply)
	if err != nil {
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> " + err.Error() + " >> " + fmtDebugStack(string(debug.Stack())))
